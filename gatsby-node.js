...
exports.sourceNodes = async ({ boundActionCreators, store, cache, createNodeId }, { channelId, apiKey, maxVideos = 50 }) => {
  ...
  const createVideoNodesFromChannelId = async (channelId, apiKey) => {
    ...
    const channelResp = await api.get(`channels?part=contentDetails&id=${channelId}&key=${apiKey}`);

    const channelData = channelResp.data.items[0];
    if (!!channelData) {
      const uploadsId = get(channelData, "contentDetails.relatedPlaylists.uploads");
      let pageSize = Math.min(50, maxVideos);

      let videoResp = await api.get(`playlistItems?part=snippet%2CcontentDetails%2Cstatus&maxResults=${pageSize}&playlistId=${uploadsId}&key=${apiKey}`);
      ...
      while (videoResp.data.nextPageToken && videos.length < maxVideos) {
        ...
        videoResp = await api.get(`playlistItems?part=snippet%2CcontentDetails%2Cstatus&maxResults=${pageSize}&pageToken=${nextPageToken}&playlistId=${uploadsId}&key=${apiKey}`);
        videos.push(...videoResp.data.items);
      }
    }
